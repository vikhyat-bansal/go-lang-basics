...
func (c *Client) Init() error {
	if err := c.checkPSQLAvailable(); err != nil {
		return err
	}
...
func (c *Client) checkPSQLAvailable() error {
	if _, err := exec.LookPath("psql"); err != nil {
		if errors.Is(err, exec.ErrNotFound) {
			return errors.New("psql executable was not found in PATH; install PostgreSQL client tools or update PATH")
		}
		return fmt.Errorf("failed to locate psql executable: %w", err)
	}
	return nil
}
